...
const FeaturesSection = (): React.JSX.Element => {
  return (
    <section className="bg-white py-16 px-6 sm:px-12 md:px-16 lg:px-32 w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-20 max-w-6xl mx-auto px-2">
        {featuresData.map((feature) => (
          ...
          >
            ...
            */}
            <div className={`absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2 ${feature.iconBg} size-14 sm:size-16 rounded-full flex items-center justify-center shadow-md flex-shrink-0`}>
              {feature.icon}
            </div>

            {/* Content Body Layout */}
            <h3 className="text-[#2F327D] font-semibold text-lg sm:text-xl lg:text-[22px] leading-snug mb-5 max-w-[240px] mx-auto min-h-[56px] flex items-center justify-center">
              {feature.title}
            </h3>

            <p className="text-[#696F8C] text-xs sm:text-sm lg:text-[15px] leading-relaxed max-w-[300px]">
              {feature.description}
            </p>
          </div>
        ))}
