...
const SuccessSection = (): React.JSX.Element => {
  return (
    <section className="bg-white py-16 px-6 sm:px-12 md:px-24 w-full text-center">
      ...
      {/*
        - grid-cols-2: Stacks into pairs on mobile screens to save vertical space.
        - md:grid-cols-3: Transitions into a clean triple column format on tablet viewports.
        - lg:grid-cols-5: Fits fluidly into a single straight row on large desktop displays.
      */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-y-12 gap-x-6 max-w-6xl mx-auto items-start justify-center">
        {successMetrics.map((metric) => (
          ...
          >
            ...
            <span className="bg-gradient-to-r from-[#23BDEE] to-[#49BBBD] bg-clip-text text-transparent font-medium text-4xl sm:text-5xl lg:text-[60px] leading-none tracking-tight">
              {metric.value}
            </span>
